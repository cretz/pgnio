...
public class QueryBuildTest extends DbTestBase {
  ...
  public void testMultipleBind() {
    withConnectionSync(c ->
        c.simpleQueryExec("CREATE TEMP TABLE testMultipleBind (foo VARCHAR(255))").
  ...
  public void testReusePrepared() {
    withConnectionSync(c ->
        c.simpleQueryExec("CREATE TEMP TABLE testReusePrepared (foo VARCHAR(255))").
            thenCompose(conn ->
  ...
  public void testReuseBound() {
    withConnectionSync(c ->
        c.simpleQueryExec("CREATE TEMP TABLE testReuseBound (foo VARCHAR(255))").
  ...
  public void testBoundExecuteMax() {
    ...
    withConnectionSync(c ->
        c.simpleQueryExec("CREATE TEMP TABLE testBoundExecuteMax (foo INTEGER); " + inserts).
            thenCompose(conn -> conn.beginTransaction()).
